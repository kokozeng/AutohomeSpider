...
def get_car_img(url, car_name):
    ...
    #print soup
    #car_list = soup.find('div', attrs={'class': 'cartab-title'})
    #print car_list
    car_list = soup.find('div', attrs={'class': 'uibox-con carpic-list03 border-b-solid'})
    #print (car_list)
...
def count_page(url):
    source = get_GBK_page_html(url)
    soup = BeautifulSoup(source, 'lxml')
    pagecont = soup.find('div', attrs={'class': 'pagecont'})
...
## 得到了car_id之后，用这个程序把每一辆车的链接抓下来，并保存成一个txt文件。注意有v1和v2两个版本，因为有一些车型他们的网页源代码class定义的不同。
filename = 'car_id.txt'
